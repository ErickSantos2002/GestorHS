...
def test_marcas_read_interno_write_admin(client, usuario_admin, usuario_comum):
    assert client.get("/marcas", headers=_headers(client, "comum@hs.com", "senha123")).status_code == 200
    assert client.post("/marcas", json={"descricao": "X"}, headers=_headers(client, "comum@hs.com", "senha123")).status_code == 403
