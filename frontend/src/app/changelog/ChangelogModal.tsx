...
export function ChangelogModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg max-h-[85vh] flex flex-col rounded-2xl bg-background-surface border border-border shadow-2xl">
        <div className="flex items-start justify-between px-5 py-4 border-b border-border shrink-0">
          <div>
            <h2 className="text-base font-bold text-slate-100">O que há de novo?</h2>
            <p className="text-xs text-slate-500 mt-0.5">Atualizações recentes do GestorHS</p>
          </div>
          <button
            onClick={onClose}
            ...
            <section key={v.versao} className="space-y-3">
              ...
              <div className="space-y-2">
                {v.itens.map((item, i) => {
                  ...
                  return (
                    <div key={i} className="flex gap-3 rounded-xl bg-background-elevated border border-border px-3.5 py-3">
