...
- ⚠️ **O bloco `destinatario` SUBSTITUI o cadastro inteiro, inclusive a matriz.** O campo "Cliente matriz" é editável no modal da proposta tanto em `nova_empresa` quanto numa Empresa **já cadastrada** (set/2026) — é de lá que vem a frota, e filial sem matriz não aceita aparelho, então resolver isso exigia sair da proposta. Consequência: `matriz` ausente no payload **desvincula** a filial (`empresas.cliente = NULL`), igual a nome ou e-mail em branco. Trocar a matriz move `propostas.cliente` junto e o frontend limpa os aparelhos marcados, que eram de outra frota. A **inscrição estadual** continua só na página Empresas.
...
- **Empresa parada em `pendente` é reenviada sozinha:** `app/tarefas/tiny_pendentes.py` cria uma task de fundo no lifespan que varre a cada **10 min** (`JOB_TINY_INTERVALO_MIN`) as Empresas ativas com `tiny_status = 'pendente'`, até **20 por volta** (`JOB_TINY_LIMITE`), 7s entre elas — cada empresa gasta 2 das 20 chamadas/minuto da conta. Sem pendência, sai sem falar com o Tiny. Nasce DESLIGADO: ligue com `JOB_TINY_ATIVO=true`, e ele também não sobe sem `TINY_TOKEN`. ⚠️ **Só `pendente`** — `erro` é recusa que não muda sozinha (repetir gastaria chamada para sempre) e `tiny_status` nulo é cadastro anterior à integração, que continua sendo do script de carga.
- ⚠️ **A trava do sync é `FOR UPDATE OF empresas`, não `FOR UPDATE` cru.** `Empresa.matriz_rel` é `lazy="joined"`, então toda leitura de Empresa sai com `LEFT OUTER JOIN clientes` e o Postgres recusa (`FOR UPDATE cannot be applied to the nullable side of an outer join`). Isso derrubou **todo** espelhamento em tempo real de 16 a 17/09/2026 — criar, editar e o botão Reenviar — e o `except` best-effort engolia a falha, deixando a empresa em `pendente` para sempre (indistinguível de "na fila" na tela). **O SQLite ignora `FOR UPDATE`, então nenhum teste de comportamento pega**: o teste é `stmt_travar_empresa` compilada no dialeto do Postgres (`test_tiny_client.py`).
...
- ⚠️ **Caixa arquivada (`fase is None`) é recusada ANTES da trava de Phoebus**, com a mensagem de fase errada (`"caixa nao esta em Pos-Vendas"`) e não a de Phoebus — senão `_ordens_ativas` devolveria lista vazia e a trava de escopo responderia "sem Phoebus" para uma caixa que na verdade foi cancelada, mensagem enganosa. Os demais casos de fase errada (ex.: caixa em Laboratório) passam pela trava de Phoebus primeiro.
